    ...
    <div>
      <div style={{ background: 'linear-gradient(145deg, var(--navy), var(--navy-mid))', padding: '40px 24px 32px' }}>
        <div style={{ maxWidth: 1160, margin: '0 auto' }}>
          <Link to="/epreuve/comprehension-ecrite" className="btn-back">&larr; Retour</Link>
          <h1 style={{ fontSize: 36, fontWeight: 900, color: 'white', letterSpacing: '-0.03em', margin: '0 0 6px' }}>Séries Compréhension Écrite</h1>
                ...
                <div key={s.id} className="serie-card">
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 14 }}>
                    <div>
                      ...
                      <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-1)', display: 'flex', alignItems: 'center', gap: 6 }}>
                        <BookOpen size={14} style={{ color: 'var(--orange)', flexShrink: 0 }} /> CE test {s.order_index}
                      </div>
                    </div>
                    {res && (
                  ...
                  {res ? (
                    <div style={{ display: 'flex', gap: 8 }}>
                      ...
                      <Link to={`/epreuve/comprehension-ecrite/entrainement/${s.slug}`} className="btn-navy-flex">Recommencer</Link>
                    </div>
                  ) : (
                    <Link to={`/epreuve/comprehension-ecrite/entrainement/${s.slug}`} className="btn-navy">
                      Commencer →
                    </Link>
                  )}
                </div>
