...
export default function CESeriesPage() {
  ...
  const handleViewResult = async (s, res) => {
    ...
    const { data: questions } = await supabase.from('questions_ce').select('*').eq('series_id', s.id).order('order_index')
    setLoadingResult(null)
    navigate(`/epreuve/comprehension-ecrite/resultats/${res.id}`, {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      const { data: s } = await supabase.from('series_ce').select('*').order('order_index')
      setSeries(s || [])
      if (user) {
        const { data: r } = await supabase.from('user_results').select('*').eq('user_id', user.id).eq('table_type', 'ce')
        setResults(r || [])
      }
      setLoading(false)
    }
    ...
    <div>
      <div style={{ background: 'linear-gradient(145deg, var(--navy), var(--navy-mid))', padding: '40px 24px 32px' }}>
        <div style={{ maxWidth: 1160, margin: '0 auto' }}>
          <Link to="/epreuve/comprehension-ecrite" style={{ background: 'oklch(100% 0 0 / 0.15)', color: 'white', borderRadius: 8, padding: '6px 14px', fontSize: 13, fontWeight: 600, display: 'inline-block', marginBottom: 16, textDecoration: 'none' }}>&larr; Retour</Link>
          <h1 style={{ fontSize: 36, fontWeight: 900, color: 'white', letterSpacing: '-0.03em', marginBottom: 6, margin: '0 0 6px' }}>Series Comprehension Ecrite</h1>
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              {[['all', 'Toutes'], ['todo', 'Non faites'], ['done', 'Terminees']].map(([val, label]) => (
                <button key={val} onClick={() => setFilter(val)} style={{
                  ...
                  background: filter === val ? 'var(--ce-main)' : 'var(--white)',
                ...
                onMouseLeave={e => { e.currentTarget.style.boxShadow = 'var(--shadow-sm)'; e.currentTarget.style.transform = 'none' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 14 }}>
                    <div>
                      ...
                      <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-1)', display: 'flex', alignItems: 'center', gap: 6 }}><BookOpen size={14} style={{ display: 'inline', flexShrink: 0 }} /> CE test {s.order_index}</div>
                    </div>
                    {res && (
                      <div style={{ background: scorePct >= 80 ? 'var(--ce-light)' : 'var(--eo-light)', color: scorePct >= 80 ? 'var(--ce-text)' : 'var(--eo-text)', fontWeight: 800, fontSize: 13, padding: '4px 10px', borderRadius: 8 }}>{scorePct}%</div>
                    )}
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 14 }}>
                    <span style={{ fontSize: 12, color: 'var(--text-3)' }}>39 questions</span>
                    {res && <span style={{ fontSize: 12, color: 'var(--ce-text)', fontWeight: 600, display: 'inline-flex', alignItems: 'center', gap: 3 }}><Check size={12} /> {res.score}/{res.total || 699} pts</span>}
                  </div>
                  {res && (
                    <div style={{ background: 'var(--surface-2)', borderRadius: 999, height: 4, overflow: 'hidden', marginBottom: 14 }}>
                      <div style={{ width: `${scorePct}%`, height: '100%', background: scorePct >= 80 ? 'var(--ce-main)' : 'var(--eo-main)', borderRadius: 999 }} />
                    </div>
                  )}
                  {res ? (
                    <div style={{ display: 'flex', gap: 8 }}>
                      ...
                      <Link to={`/epreuve/comprehension-ecrite/entrainement/${s.slug}`} style={{
                        flex: 1, background: 'var(--ce-main)', color: 'white', border: 'none', borderRadius: 8, padding: 9, fontSize: 13, fontWeight: 700, textAlign: 'center', textDecoration: 'none',
                      }}>Recommencer</Link>
                    </div>
                  ) : (
                    <Link to={`/epreuve/comprehension-ecrite/entrainement/${s.slug}`} style={{
                      display: 'block', width: '100%', background: 'var(--ce-main)', color: 'white', borderRadius: 8, padding: 9, fontSize: 13, fontWeight: 700, textAlign: 'center', textDecoration: 'none',
                    }}>Commencer &rarr;</Link>
                  )}
                </div>
              )
