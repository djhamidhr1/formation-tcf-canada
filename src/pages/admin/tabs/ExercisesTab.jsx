  ...
  return (
    <div className="relative">
      ...
      <input type="text" value={value} onChange={e=>onChange(e.target.value)}
        placeholder="Rechercher…" className="pl-8 pr-3 py-2 text-sm border border-gray-200 rounded-xl w-full focus:outline-none focus:ring-2 focus:ring-blue-300"/>
    </div>
  )
}
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="px-5 py-3 border-b border-gray-100 flex items-center gap-3 flex-wrap">
          ...
          <button onClick={()=>{ setSf({ title:'', slug:'', order_index: (series.length+1)+'' }); setModal('addSeries') }}
            className="flex items-center gap-1.5 bg-[#0F3D58] text-white px-3 py-2 rounded-xl text-xs font-bold hover:bg-[#F98012] hover:text-white">
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-50">
                ...
                {filteredSeries.map(s => (
                  <tr key={s.id} className={`hover:bg-orange-50/30 transition-colors cursor-pointer ${selectedSeries?.id===s.id?'bg-orange-50 border-l-2 border-[#F98012]':''}`}
                    onClick={()=>setSelectedSeries(selectedSeries?.id===s.id?null:s)}>
                    <td className="px-4 py-2.5">
                      <span className="bg-orange-100 text-[#F98012] font-extrabold text-xs px-2 py-0.5 rounded-lg">Test {s.order_index}</span>
                    </td>
        ...
        <div className="bg-white rounded-2xl border border-[#e8e0d8] shadow-sm overflow-hidden">
          <div className="px-5 py-3 border-b border-[#e8e0d8] bg-[#FDF2E9] flex items-center gap-3 flex-wrap">
            ...
            <div>
              ...
              <p className="text-xs text-[#0F3D58]">{questions.length} question{questions.length!==1?'s':''} · {isCO?'39 attendues':'39 attendues'}</p>
            </div>
            <div className="flex-1 min-w-[160px]"><SearchBox value={qSearch} onChange={setQSearch}/></div>
            <button onClick={openAddQ} className="flex items-center gap-1.5 bg-[#0F3D58] text-white px-3 py-2 rounded-xl text-xs font-bold hover:bg-[#F98012]">
                  ...
                  {filteredQ.map(q => {
                    ...
                    return (
                      <tr key={q.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-2.5">
                          <div className="flex items-center gap-1 justify-end">
                            <button onClick={()=>openEditQ(q)} className="p-1.5 rounded-lg hover:bg-orange-50 text-gray-400 hover:text-[#F98012]"><Edit2 size={13}/></button>
          ...
          <div className="space-y-3">
            <div>
              ...
              <input type="number" value={sf.order_index} onChange={e=>setSf(f=>({...f,order_index:e.target.value}))}
                className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-300" placeholder="ex: 41"/>
            </div>
            <div>
            ...
            <div className="flex gap-3 justify-end pt-2">
              ...
              <button onClick={saveSeries} className="flex items-center gap-1.5 px-4 py-2 text-sm bg-[#0F3D58] text-white rounded-xl font-bold hover:bg-[#F98012] hover:text-white">
      ...
      {(modal==='addQ'||modal==='editQ') && (
        <Modal title={modal==='addQ'?'Nouvelle question':'Modifier la question'} onClose={()=>setModal(null)} wide>
          <div className="space-y-4">
              ...
              <>
                <div>
                  ...
                  <input type="text" value={qf.prompt} onChange={e=>setQf(f=>({...f,prompt:e.target.value}))}
                    className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-300" placeholder="Ex: Vous entendez une conversation entre..."/>
                </div>
                {/* Audio upload */}
                <div>
                  ...
                  <div className="flex items-center gap-3 flex-wrap">
                    ...
                    <input type="text" value={qf.audio_url} onChange={e=>setQf(f=>({...f,audio_url:e.target.value}))}
                      className="flex-1 min-w-[180px] border border-gray-200 rounded-xl px-3 py-2 text-xs focus:outline-none focus:ring-2 focus:ring-blue-300 font-mono" placeholder="URL audio…"/>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <input type="text" value={qf.image_url} onChange={e=>setQf(f=>({...f,image_url:e.target.value}))}
                      className="flex-1 border border-gray-200 rounded-xl px-3 py-2 text-xs focus:outline-none focus:ring-2 focus:ring-blue-300 font-mono" placeholder="URL image…"/>
                  </div>
                ...
                {qf.options.map((opt,i)=>(
                  <div key={i} className="flex items-center gap-2">
                    ...
                    <input type="text" value={opt} onChange={e=>setQf(f=>{ const o=[...f.options]; o[i]=e.target.value; return {...f,options:o} })}
                      className={`flex-1 border rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 ${qf.correct_answer_index===i?'border-green-300 bg-green-50 focus:ring-green-300':'border-gray-200 focus:ring-blue-300'}`}
            ...
            <div className="flex gap-3 justify-end pt-2">
              <button onClick={()=>setModal(null)} className="px-4 py-2 text-sm border border-gray-200 rounded-xl hover:bg-gray-50">Annuler</button>
              <button onClick={saveQuestion} className="flex items-center gap-1.5 px-4 py-2 text-sm bg-[#0F3D58] text-white rounded-xl font-bold hover:bg-[#F98012] hover:text-white">
                <Save size={13}/> Enregistrer
              </button>
            </div>
          </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 flex flex-wrap gap-3 items-center">
        ...
        <button onClick={openAdd} className="flex items-center gap-1.5 bg-[#0F3D58] text-white px-3 py-2 rounded-xl text-xs font-bold hover:bg-[#F98012] hover:text-white">
        ...
        Object.keys(byYear).sort().map(year => (
          <div key={year} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <div className="px-5 py-2.5 bg-gray-50 border-b border-gray-100 flex items-center gap-2">
              ...
              <span className="bg-orange-100 text-[#F98012] text-xs font-bold px-2 py-0.5 rounded-full">{byYear[year].length} combinaison{byYear[year].length!==1?'s':''}</span>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-gray-50">
                  {byYear[year].map((it,i) => (
                    <tr key={it.id} className="hover:bg-gray-50">
                      <td className="px-4 py-2.5">
                        <span className="bg-orange-100 text-[#F98012] text-xs font-bold px-2 py-0.5 rounded-lg">{fmtMonth(it.month_slug)}</span>
                      ...
                      <td className="px-4 py-2.5">
                        <div className="flex items-center gap-1 justify-end">
                          <button onClick={()=>openEdit(it)} className="p-1.5 rounded-lg hover:bg-orange-50 text-gray-400 hover:text-[#F98012]"><Edit2 size={13}/></button>
          ...
          <div className="space-y-4">
            <div>
              ...
              <input type="text" value={form.month_slug} onChange={e=>setForm(f=>({...f,month_slug:e.target.value}))}
                className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-300" placeholder="2026-06"/>
            </div>
            {[
            ...
            <div className="flex gap-3 justify-end pt-2">
              <button onClick={()=>setModal(null)} className="px-4 py-2 text-sm border border-gray-200 rounded-xl hover:bg-gray-50">Annuler</button>
              <button onClick={save} className="flex items-center gap-1.5 px-4 py-2 text-sm bg-[#0F3D58] text-white rounded-xl font-bold hover:bg-[#F98012] hover:text-white">
                <Save size={13}/> Enregistrer
              </button>
            </div>
          </div>
...
function EOSection() {
  ...
  const TACHE_COLORS = { 1:'bg-orange-100 text-[#F98012]', 2:'bg-green-100 text-green-700', 3:'bg-orange-100 text-orange-700' }
      ...
      {loading ? <div className="flex justify-center py-8"><div className="w-6 h-6 border-2 border-[#71C9CE] border-t-transparent rounded-full animate-spin"/></div> : (
        Object.keys(byYear).sort().map(year => (
          <div key={year} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <div className="px-5 py-2.5 bg-gray-50 border-b border-gray-100 flex items-center gap-2">
              <span className="font-extrabold text-gray-700 text-sm flex items-center gap-1"><Calendar size={13} className="shrink-0" /> {year}</span>
              <span className="bg-orange-100 text-[#F98012] text-xs font-bold px-2 py-0.5 rounded-full">{byYear[year].length} sujet{byYear[year].length!==1?'s':''}</span>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="border-b border-gray-100">
                ...
                <tbody className="divide-y divide-gray-50">
                  {byYear[year].map(it => (
                    <tr key={it.id} className="hover:bg-gray-50">
                      <td className="px-4 py-2.5"><span className="bg-orange-100 text-[#F98012] text-xs font-bold px-2 py-0.5 rounded-lg">{fmtMonth(it.month_slug)}</span></td>
