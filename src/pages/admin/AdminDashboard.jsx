...
/* ═══════════════════════════════════════════════════════════
   TAB 2 — EXERCICES CRUD
═══════════════════════════════════════════════════════════ */
function ExercisesTab() {
  const [section, setSection] = useState('ee')   // ce | co | ee | eo
  ...
  const [form, setForm] = useState({ title: '', month_slug: '', tache1_sujet: '', tache1_correction: '', tache2_sujet: '', tache2_correction: '', tache3_titre: '', tache3_document1: '', tache3_document2: '', tache3_correction: '', audio_url: '' })

  const TABLE_MAP = { ce: 'series_ce', co: 'series_co', ee: 'combinaisons_ee', eo: 'sujets_eo' }

  const load = useCallback(async () => {
    setLoading(true)
    const table = TABLE_MAP[section]
    const q = supabase.from(table).select('*').order(section === 'ee' ? 'month_slug' : 'order_index', { ascending: false })
    const { data } = await q
    setItems(data || [])
    setLoading(false)
  }, [section])
  ...
  const filtered = items.filter(it => {
    ...
    return (
      ...
      it.month_slug?.toLowerCase().includes(s) ||
      it.slug?.toLowerCase().includes(s) ||
      it.tache1_sujet?.toLowerCase().includes(s)
    )
  })
  ...
  const openAdd = () => {
    setSelected(null)
    setForm({ title: '', month_slug: '', tache1_sujet: '', tache1_correction: '', tache2_sujet: '', tache2_correction: '', tache3_titre: '', tache3_document1: '', tache3_document2: '', tache3_correction: '', audio_url: '' })
    setModal('add')
  }
  ...
  const handleAudioUpload = async (e) => {
    ...
    const { error } = await supabase.storage.from('audios-co').upload(path, file, { upsert: true })
    if (error) { toast.error('Erreur upload: ' + error.message) }
    else {
      const { data: { publicUrl } } = supabase.storage.from('audios-co').getPublicUrl(path)
  ...
  const handleSave = async () => {
    ...
    if (modal === 'add') {
      ...
      const { error } = await supabase.from(table).update(form).eq('id', selected.id)
      if (error) { toast.error(error.message); return }
  ...
  const SECTIONS = [
    { id: 'ee', label: 'Expression Écrite', icon: FileText },
    { id: 'ce', label: 'Compréhension Écrite', icon: BookOpen },
    { id: 'co', label: 'Compréhension Orale', icon: Mic },
    { id: 'eo', label: 'Expression Orale', icon: Volume2 },
  ]
    ...
    <div className="space-y-5">
      ...
      <div className="flex gap-2 flex-wrap">
        {SECTIONS.map(({ id, label, icon: Icon }) => (
          <button key={id} onClick={() => setSection(id)}
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold transition-colors ${section === id ? 'bg-[#7D3C98] text-white' : 'bg-white border border-gray-200 text-gray-600 hover:bg-gray-50'}`}>
            <Icon size={14} />{label}
          </button>
        ))}
      </div>
      ...
      <div className="flex gap-3 items-center flex-wrap">
        <div className="flex-1 min-w-[200px]">
          <SearchInput value={search} onChange={setSearch} placeholder={`Chercher dans ${SECTIONS.find(s => s.id === section)?.label}...`} />
        </div>
        ...
        <button onClick={openAdd} className="flex items-center gap-2 bg-[#7D3C98] text-white px-4 py-2 rounded-xl text-sm font-bold hover:bg-[#6C3483]">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr>
                <th className="px-4 py-3 text-left text-xs font-extrabold text-gray-500 uppercase tracking-wide">#</th>
                {section === 'ee' && <th className="px-4 py-3 text-left text-xs font-extrabold text-gray-500 uppercase tracking-wide">Mois</th>}
                {section !== 'ee' && <th className="px-4 py-3 text-left text-xs font-extrabold text-gray-500 uppercase tracking-wide">Titre / Slug</th>}
                <th className="px-4 py-3 text-left text-xs font-extrabold text-gray-500 uppercase tracking-wide">Aperçu</th>
                <th className="px-4 py-3 text-left text-xs font-extrabold text-gray-500 uppercase tracking-wide">Créé le</th>
                <th className="px-4 py-3 text-right text-xs font-extrabold text-gray-500 uppercase tracking-wide">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-50">
              ...
              {!loading && filtered.length === 0 && (
                <tr><td colSpan={5} className="px-4 py-10 text-center text-gray-400 text-sm">Aucun exercice trouvé</td></tr>
              )}
              {!loading && filtered.map((item, i) => (
                <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-4 py-3 text-gray-400 text-xs">{i + 1}</td>
                  <td className="px-4 py-3 font-semibold text-gray-800 max-w-[200px] truncate">
                    {item.month_slug || item.title || item.slug || '—'}
                  </td>
                  <td className="px-4 py-3 text-gray-500 text-xs max-w-[250px] truncate">
                    {item.tache1_sujet?.substring(0, 80) || item.content_html?.substring(0, 80) || item.title || '—'}
                  </td>
                  <td className="px-4 py-3 text-gray-400 text-xs">{fmt(item.created_at)}</td>
                  <td className="px-4 py-3 text-right">
                    <div className="flex items-center gap-2 justify-end">
                      <button onClick={() => openEdit(item)} className="p-1.5 rounded-lg hover:bg-purple-50 text-gray-400 hover:text-purple-600"><Edit2 size={14} /></button>
                      <button onClick={() => { setSelected(item); setModal('delete') }} className="p-1.5 rounded-lg hover:bg-red-50 text-gray-400 hover:text-red-500"><Trash2 size={14} /></button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      {(modal === 'add' || modal === 'edit') && (
        <Modal title={modal === 'add' ? 'Nouvel exercice' : 'Modifier l\'exercice'} onClose={() => setModal(null)} wide>
          <div className="space-y-4">
            {/* Common fields */}
            {section === 'ee' && (
              <div>
                <label className="block text-xs font-bold text-gray-700 mb-1">Mois (ex: 2026-03)</label>
            ...
            {section !== 'ee' && (
              <>
                ...
                <div>
                  <label className="block text-xs font-bold text-gray-700 mb-1">Numéro d'ordre</label>
                  <input type="number" value={form.order_index || ''} onChange={e => setForm(f => ({ ...f, order_index: parseInt(e.target.value) }))}
                    className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-purple-300" />
                </div>
              </>
            )}
            ...
            {section === 'co' && (
              <div>
                <label className="block text-xs font-bold text-gray-700 mb-1">Audio (MP3)</label>
                <div className="flex items-center gap-3">
                  <label className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold cursor-pointer transition-colors ${uploading ? 'bg-gray-100 text-gray-400' : 'bg-purple-50 text-purple-700 hover:bg-purple-100'}`}>
                    <Upload size={14} />
                  ...
                  {form.audio_url && (
                    <div className="flex items-center gap-2 text-xs text-green-600 font-semibold">
                      <CheckCircle size={14} /> Audio chargé
                    </div>
                  )}
                </div>
                {form.audio_url && (
                  <audio controls src={form.audio_url} className="mt-2 w-full h-8" />
                )}
              </div>
            )}
            ...
            {section === 'ee' && (
              <>
                ...
                <div className="border border-blue-200 rounded-xl p-4 bg-blue-50/30">
                  <h4 className="text-sm font-extrabold text-blue-700 mb-3">Tâche 1 — Message court (60-120 mots)</h4>
                  <div className="space-y-3">
                    <div>
                      <label className="block text-xs font-bold text-gray-700 mb-1">Sujet</label>
                      <RichEditor value={form.tache1_sujet || ''} onChange={v => setForm(f => ({ ...f, tache1_sujet: v }))} placeholder="Sujet de la tâche 1..." />
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-gray-700 mb-1">Correction suggérée</label>
                      <RichEditor value={form.tache1_correction || ''} onChange={v => setForm(f => ({ ...f, tache1_correction: v }))} placeholder="Correction pour la tâche 1..." />
                    </div>
                  </div>
                </div>
                {/* Tâche 2 */}
                <div className="border border-green-200 rounded-xl p-4 bg-green-50/30">
                  <h4 className="text-sm font-extrabold text-green-700 mb-3">Tâche 2 — Narration (120-150 mots)</h4>
                  <div className="space-y-3">
                    <div>
                      <label className="block text-xs font-bold text-gray-700 mb-1">Sujet</label>
                      <RichEditor value={form.tache2_sujet || ''} onChange={v => setForm(f => ({ ...f, tache2_sujet: v }))} placeholder="Sujet de la tâche 2..." />
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-gray-700 mb-1">Correction suggérée</label>
                      <RichEditor value={form.tache2_correction || ''} onChange={v => setForm(f => ({ ...f, tache2_correction: v }))} placeholder="Correction pour la tâche 2..." />
                    </div>
                  </div>
                </div>
                {/* Tâche 3 */}
                <div className="border border-orange-200 rounded-xl p-4 bg-orange-50/30">
                  <h4 className="text-sm font-extrabold text-orange-700 mb-3">Tâche 3 — Synthèse (150-200 mots)</h4>
                  <div className="space-y-3">
                    <div>
                      <label className="block text-xs font-bold text-gray-700 mb-1">Titre / Contexte</label>
                      <input type="text" value={form.tache3_titre || ''} onChange={e => setForm(f => ({ ...f, tache3_titre: e.target.value }))}
                        className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-300" />
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-gray-700 mb-1">Document 1</label>
                      <RichEditor value={form.tache3_document1 || ''} onChange={v => setForm(f => ({ ...f, tache3_document1: v }))} placeholder="Document source 1..." />
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-gray-700 mb-1">Document 2</label>
                      <RichEditor value={form.tache3_document2 || ''} onChange={v => setForm(f => ({ ...f, tache3_document2: v }))} placeholder="Document source 2..." />
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-gray-700 mb-1">Correction suggérée</label>
                      <RichEditor value={form.tache3_correction || ''} onChange={v => setForm(f => ({ ...f, tache3_correction: v }))} placeholder="Correction pour la tâche 3..." />
                    </div>
                  </div>
                </div>
              </>
            ...
            <div className="flex gap-3 justify-end pt-2">
              ...
              <button onClick={handleSave} className="flex items-center gap-2 px-5 py-2 text-sm bg-[#7D3C98] text-white rounded-xl font-bold hover:bg-[#6C3483]">
      ...
      {modal === 'delete' && (
        <Confirm
          message={`Supprimer définitivement cet exercice ? Cette action est irréversible.`}
